...
wptoggle () {
    ...
    if [[ "$1" == "Discharging" ]] && [[ "$1" != "$laststate" ]]; then
      ...
      pgrep mpvpaper || (for monitor in "${monitors[@]}"; do mpvpaper -spf -n 300 -o '--speed=0.5 --ao=null --reset-on-next-file=loop --loop-playlist --shuffle --panscan=1 --hwdec=auto input-ipc-server=/tmp/mpv-socket' "$monitor" "$videos"; done)
